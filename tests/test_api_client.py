...
class MyAppClient(APIClient):
    """A concrete `APIClient` subclass declaring `app_name`, for tests that need a constructible client"""

    app_name = "myapp"


class TestAPIClientAppName:
    """Tests for `APIClient.app_name` validation in `__init__`"""

    def test_bare_api_client_raises_type_error(self) -> None:
        """Test that constructing the base `APIClient` directly raises TypeError, since it sets no `app_name`"""
        with pytest.raises(TypeError, match="must set a non-empty 'app_name' class attribute"):
            APIClient(base_url=BASE_URL)

    def test_subclass_with_no_app_name_raises_type_error(self) -> None:
        """Test that a subclass declaring no `app_name` raises TypeError on construction"""

        class NoNameClient(APIClient):
            pass

        with pytest.raises(TypeError, match="must set a non-empty 'app_name' class attribute"):
            NoNameClient(base_url=BASE_URL)

    def test_subclass_with_empty_app_name_raises_type_error(self) -> None:
        """Test that a subclass declaring an empty string `app_name` raises TypeError on construction"""

        class EmptyNameClient(APIClient):
            app_name = ""

        with pytest.raises(TypeError, match="must set a non-empty 'app_name' class attribute"):
            EmptyNameClient(base_url=BASE_URL)

    def test_subclass_with_non_str_app_name_raises_type_error(self) -> None:
        """Test that a subclass declaring a non-str `app_name` raises TypeError on construction"""

        class NonStrNameClient(APIClient):
            app_name = 123

        with pytest.raises(TypeError, match="must set a non-empty 'app_name' class attribute"):
            NonStrNameClient(base_url=BASE_URL)

    def test_subclass_with_a_declared_app_name_constructs(self) -> None:
        """Test that a subclass declaring a non-empty `app_name` constructs successfully and exposes it"""
        client = MyAppClient(base_url=BASE_URL)
...
class TestAPIClientInit:
    ...
    async def test_init_with_base_url_creates_async_rest_client_in_async_mode(self) -> None:
        ...
        client = MyAppClient(base_url=BASE_URL, async_mode=True)
    ...
    def test_init_raises_when_both_base_url_and_rest_client_provided(self) -> None:
        ...
        with pytest.raises(ValueError, match="base_url is not supported when rest_client is provided"):
            MyAppClient(base_url=BASE_URL, rest_client=rest_client)
    ...
    def test_init_raises_when_sync_rest_client_given_in_async_mode(self) -> None:
        ...
        with pytest.raises(TypeError, match=f"rest_client must be of type {AsyncRestClient.__name__}"):
            MyAppClient(rest_client=rest_client, async_mode=True)
    ...
    def test_init_raises_when_async_rest_client_given_in_sync_mode(self) -> None:
        ...
        try:
            with pytest.raises(TypeError, match=f"rest_client must be of type {RestClient.__name__}"):
                MyAppClient(rest_client=rest_client, async_mode=False)
        finally:
    ...
    def test_init_with_rest_client_sets_base_url_from_rest_client(self) -> None:
        ...
        rest_client = RestClient(BASE_URL)
        client = MyAppClient(rest_client=rest_client)
        assert client.base_url == BASE_URL
    ...
    def test_env_stored_when_provided(self) -> None:
        ...
        client = MyAppClient(base_url=BASE_URL, env="dev")
...
class TestAPIClientContextManager:
    ...
    def test_sync_context_manager_closes_rest_client(self, mocker: MockerFixture) -> None:
        ...
        with MyAppClient(base_url=BASE_URL) as client:
    ...
    async def test_async_context_manager_closes_rest_client(self, mocker: MockerFixture) -> None:
        ...
        async with MyAppClient(rest_client=rest_client, async_mode=True) as client:
            assert isinstance(client, APIClient)
...
class TestAPIClientClose:
    ...
    async def test_aclose_delegates_to_rest_client(self, mocker: MockerFixture) -> None:
        ...
        rest_client = AsyncRestClient(BASE_URL)
        mocker.patch.object(rest_client, "close", return_value=None)
        client = MyAppClient(rest_client=rest_client, async_mode=True)
        await client.aclose()
        rest_client.close.assert_called_once()
    ...
    def test_context_manager_exit_calls_close(self, mocker: MockerFixture) -> None:
        ...
        with MyAppClient(base_url=BASE_URL):
            pass
        mock_close.assert_called_once()
    ...
    async def test_async_context_manager_exit_calls_aclose(self, mocker: MockerFixture) -> None:
        ...
        async with MyAppClient(rest_client=rest_client, async_mode=True):
            pass

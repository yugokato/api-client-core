"""Unit tests for core endpoints/utils/endpoint_call.py"""
...
from api_client_core.types import Alias, EndpointModel, File, Query, Unset


class _FakeAPIClass:
    """Minimal API class stand-in — only __name__ is required by validate_params."""
...
def _make_endpoint(
    ...
    *,
    ...
    field_list = [
        (name, tp, field(default=Unset, metadata=field_metadata.get(name, {}))) for name, tp in (fields or {}).items()
    ]
...
class TestSplitParams:
    ...
    def test_path_and_body_params_are_separated(self) -> None:
        ...
        path_params, body_params = endpoint_call_util.split_params(
            _func, "/v1/users/{user_id}", ("u123",), {"name": "Alice"}
        )
        assert path_params == ("u123",)
        assert body_params == {"name": "Alice"}

    def test_multiple_path_params_are_returned_in_placeholder_order(self) -> None:
        """Test that resolved path params are returned as a tuple ordered by placeholder position"""

        def _func(self: Any, user_id: str, order_id: str, name: str) -> None: ...

        path_params, body_params = endpoint_call_util.split_params(
            _func, "/v1/users/{user_id}/orders/{order_id}", ("u123", "o456"), {"name": "Alice"}
        )
        assert path_params == ("u123", "o456")
        assert body_params == {"name": "Alice"}
    ...
    def test_var_keyword_params_are_flattened_into_body(self) -> None:
        ...
        path_params, body_params = endpoint_call_util.split_params(
            _func, "/v1/test", (), {"name": "Alice", "extra": "ext"}
        )
        assert path_params == ()
    ...
    def test_path_default_is_applied_when_caller_omits_path_param(self) -> None:
        ...
        path_params, body_params = endpoint_call_util.split_params(_func, "/v1/users/{user_id}", (), {})
        assert path_params == ("default-id",)
        assert body_params == {}
    ...
    def test_non_identifier_placeholder_accepts_cleaned_caller_name(self) -> None:
        ...
        path_params, body_params = endpoint_call_util.split_params(
            _func, "/v1/customers/{customer-id}", (), {"customer_id": "c123"}
        )
    ...
    def test_unknown_kwarg_reraises_as_natural_type_error(self) -> None:
        ...
        with pytest.raises(TypeError, match=r"_func\(\): got an unexpected keyword argument 'unknown_kwarg'"):
            endpoint_call_util.split_params(_func, "/v1/test", (), {"unknown_kwarg": "val"})
    ...
    def test_too_many_positional_args_reraises_with_function_name(self) -> None:
        ...
        with pytest.raises(TypeError, match=r"_func\(\): too many positional arguments"):
            endpoint_call_util.split_params(_func, "/v1/test", ("v1", "v2"), {})
    ...
    def test_explicit_unset_path_param_falls_back_to_default(self) -> None:
        ...
        path_params, body_params = endpoint_call_util.split_params(_func, "/v1/users/{user_id}", (), {"user_id": Unset})
        assert path_params == ("default-id",)
        assert body_params == {}

    def test_explicit_unset_path_param_without_default_raises(self) -> None:
        """Test that a path param explicitly given as Unset with no default is treated as missing and raises"""

        def _func(self: Any, user_id: str) -> None: ...

        with pytest.raises(ValueError, match=r"_func\(\) missing 1 required path parameter: 'user_id'"):
            endpoint_call_util.split_params(_func, "/v1/users/{user_id}", (), {"user_id": Unset})

    def test_missing_trailing_path_param_names_it_in_error(self) -> None:
        """Test that omitting the last placeholder raises ValueError naming that placeholder"""

        def _func(self: Any, user_id: str, order_id: str) -> None: ...

        with pytest.raises(ValueError, match=r"_func\(\) missing 1 required path parameter: 'order_id'"):
            endpoint_call_util.split_params(_func, "/v1/users/{user_id}/orders/{order_id}", (), {"user_id": "u123"})

    def test_missing_leading_path_param_names_it_not_the_provided_one(self) -> None:
        """Test that omitting an earlier placeholder while providing a later one names the actual missing one"""

        def _func(self: Any, user_id: str, order_id: str) -> None: ...

        with pytest.raises(ValueError, match=r"missing 1 required path parameter: 'user_id'"):
            endpoint_call_util.split_params(_func, "/v1/users/{user_id}/orders/{order_id}", (), {"order_id": "o456"})

    def test_missing_multiple_path_params_names_all_in_error(self) -> None:
        """Test that omitting multiple placeholders raises ValueError naming all of them, pluralized"""

        def _func(self: Any, user_id: str, order_id: str) -> None: ...

        with pytest.raises(ValueError, match=r"missing 2 required path parameters: 'user_id', 'order_id'"):
            endpoint_call_util.split_params(_func, "/v1/users/{user_id}/orders/{order_id}", (), {})


class TestNormalizeCallArgs:
    """Tests for `normalize_call_args()`

    This utility exists solely for a name-keyed caller (the CLI's `runner.py`, the MCP server's own
    dispatch) that can only ever produce keyword arguments. It must never be wired into the framework's
    own call-binding path (`split_params()`, `EndpointFunc`), which deliberately keeps enforcing
    positional-only-ness for every direct Python call.
    """
    ...
    def test_no_positional_only_params_returns_input_unchanged(self) -> None:
        ...
        args, kwargs = endpoint_call_util.normalize_call_args(_func, (), {"a": "1", "b": "2"})
    ...
    def test_positional_only_param_already_given_positionally_returns_unchanged(self) -> None:
        ...
        args, kwargs = endpoint_call_util.normalize_call_args(_func, ("1",), {"b": "2"})
    ...
    def test_required_positional_only_param_by_keyword_moves_into_args(self) -> None:
        ...
        args, kwargs = endpoint_call_util.normalize_call_args(_func, (), {"a": "7"})
    ...
    def test_gap_before_named_param_is_filled_from_its_own_default(self) -> None:
        ...
        args, kwargs = endpoint_call_util.normalize_call_args(_func, (), {"b": 5})
    ...
    def test_gap_with_no_default_raises_naming_both_params(self) -> None:
        ...
        with pytest.raises(TypeError, match=r"_func\(\).*'b'.*'a'"):
            endpoint_call_util.normalize_call_args(_func, (), {"b": "5"})
    ...
    def test_param_passed_positionally_and_by_name_with_var_keyword_returns_unchanged(self) -> None:
        ...
        args, kwargs = endpoint_call_util.normalize_call_args(_func, (1,), {"a": 9})
    ...
    def test_var_positional_param_is_untouched(self) -> None:
        ...
        args, kwargs = endpoint_call_util.normalize_call_args(_func, (), {"a": "1", "b": "5"})
    ...
    def test_does_not_mutate_the_caller_supplied_kwargs_dict(self) -> None:
        ...
        original_kwargs = {"a": "7"}
        endpoint_call_util.normalize_call_args(_func, (), original_kwargs)
    ...
    def test_is_idempotent(self) -> None:
        ...
        args1, kwargs1 = endpoint_call_util.normalize_call_args(_func, (), {"a": "7"})
        args2, kwargs2 = endpoint_call_util.normalize_call_args(_func, args1, kwargs1)
...
class TestIsJsonRequest:
    ...
    def test_request_header_matched_case_insensitively(self) -> None:
        """Test that a request `Content-Type` header is matched regardless of casing"""
        endpoint = _make_endpoint({})
        raw_options = {"headers": {"CONTENT-TYPE": "text/xml"}}
        assert endpoint_call_util.is_json_request(endpoint, {}, raw_options, {}) is False
    ...
    def test_content_type_media_type_is_matched_case_insensitively(self) -> None:
        """Test that an uppercase `Content-Type` media type is still recognized as JSON"""
        endpoint = _make_endpoint({})
        raw_options = {"headers": {"Content-Type": "APPLICATION/JSON"}}
        assert endpoint_call_util.is_json_request(endpoint, {}, raw_options, {}) is True

    def test_content_type_with_space_before_semicolon_is_parsed_correctly(self) -> None:
        """Test that a `Content-Type` with a space before the `;` separator is still recognized as JSON"""
        endpoint = _make_endpoint({})
        raw_options = {"headers": {"Content-Type": "application/json ; charset=utf-8"}}
        assert endpoint_call_util.is_json_request(endpoint, {}, raw_options, {}) is True


class TestParamLocation:
    """Tests for endpoint_call_util.param_location()'s path/query/body classification"""

    def test_path_metadata_field_is_path(self) -> None:
        """Test that a field carrying path metadata is classified as "path", regardless of method"""
        endpoint = _make_endpoint({"user_id": str}, method="POST", field_metadata={"user_id": {"path": True}})
        field_obj = endpoint.model.__dataclass_fields__["user_id"]
        assert endpoint_call_util.get_param_location(endpoint, field_obj) == "path"

    def test_get_method_parameter_is_query(self) -> None:
        """Test that every parameter on a GET endpoint is classified as "query", regardless of its own
        annotation
        """
        endpoint = _make_endpoint({"limit": int}, method="get")
        field_obj = endpoint.model.__dataclass_fields__["limit"]
        assert endpoint_call_util.get_param_location(endpoint, field_obj) == "query"

    def test_use_query_string_endpoint_parameter_is_query(self) -> None:
        """Test that every parameter is classified as "query" when the endpoint func itself uses a
        query string, regardless of method or annotation
        """
        endpoint = _make_endpoint({"name": str}, method="POST", use_query_string=True)
        field_obj = endpoint.model.__dataclass_fields__["name"]
        assert endpoint_call_util.get_param_location(endpoint, field_obj) == "query"

    def test_explicit_query_annotation_on_a_non_get_endpoint_is_query(self) -> None:
        """Test that an Annotated[T, Query()] parameter on a non-GET endpoint is still classified as
        "query"
        """
        endpoint = _make_endpoint({"notify": Annotated[bool, Query()]}, method="POST")
        field_obj = endpoint.model.__dataclass_fields__["notify"]
        assert endpoint_call_util.get_param_location(endpoint, field_obj) == "query"

    def test_ordinary_non_get_parameter_is_body(self) -> None:
        """Test that an ordinary (non-Query) parameter on a non-GET endpoint is classified as "body\""""
        endpoint = _make_endpoint({"name": str}, method="POST")
        field_obj = endpoint.model.__dataclass_fields__["name"]
        assert endpoint_call_util.get_param_location(endpoint, field_obj) == "body"


class TestCompleteEndpoint:
    """Tests for endpoint_call_util.complete_endpoint()"""
    ...
    def test_returns_completed_path(self) -> None:
        ...
        result = endpoint_call_util.complete_endpoint(endpoint, ("u123",))
    ...
    def test_path_without_placeholders_returned_unchanged(self) -> None:
        ...
        result = endpoint_call_util.complete_endpoint(endpoint, ())
        assert result == "/v1/health"


class TestValidateParams:
    """Tests for endpoint_call_util.validate_params()"""

    def test_deprecated_endpoint_logs_warning(self, mocker: MockerFixture) -> None:
        """Test that validating params for a deprecated endpoint logs a DEPRECATED warning"""
        endpoint = _make_endpoint({}, is_deprecated=True)
        mock_log = mocker.patch.object(endpoint_call_util, "logger")
        endpoint_call_util.validate_params(endpoint, {}, None)
    ...
    def test_invalid_raw_option_raises_runtime_error(self, mocker: MockerFixture) -> None:
        ...
        with pytest.raises(RuntimeError, match="Invalid raw option"):
            endpoint_call_util.validate_params(endpoint, {}, {"bad_opt": True})
    ...
    def test_unexpected_body_param_logs_warning(self, mocker: MockerFixture) -> None:
        ...
        mock_log = mocker.patch.object(endpoint_call_util, "logger")
        endpoint_call_util.validate_params(endpoint, {"unexpected_param": "val"}, None)
    ...
    def test_deprecated_param_logs_warning(self, mocker: MockerFixture) -> None:
        ...
        mock_log = mocker.patch.object(endpoint_call_util, "logger")
        endpoint_call_util.validate_params(endpoint, {"old_param": "val"}, None)
        assert mock_log.warning.called
        warning_messages = [call[0][0] for call in mock_log.warning.call_args_list]
    ...
    def test_unmodeled_params_on_multipart_endpoint_log_warning(self, mocker: MockerFixture) -> None:
        ...
        mock_log = mocker.patch.object(endpoint_call_util, "logger")
        endpoint_call_util.validate_params(
            endpoint,
            {"file": File("test.txt", b"hello", "text/plain"), "unknown_a": "val_a", "unknown_b": 123},
            None,
        )
        assert mock_log.warning.called
        warning_messages = [call[0][0] for call in mock_log.warning.call_args_list]
    ...
    def test_quiet_in_call_time_raw_options_raises_runtime_error(self) -> None:
        """Test that passing `quiet` inside call-time `raw_options` raises RuntimeError instead of later colliding
        with the `quiet` keyword `generate_rest_func_params()` always receives explicitly
        """
        endpoint = _make_endpoint({})
        with pytest.raises(RuntimeError, match="Invalid raw option"):
            endpoint_call_util.validate_params(endpoint, {}, {"quiet": True})
...
class TestValidateRawOptions:
    """Tests for endpoint_call_util.validate_raw_options()"""

    def test_supported_raw_option_does_not_raise(self) -> None:
        """Test that a raw option supported by the HTTP client is accepted"""
        endpoint_call_util.validate_raw_options({"timeout": 30})

    def test_unsupported_raw_option_raises_runtime_error(self) -> None:
        """Test that a raw option not supported by the HTTP client raises RuntimeError"""
        with pytest.raises(RuntimeError, match="Invalid raw option"):
            endpoint_call_util.validate_raw_options({"bogus": True})

    def test_quiet_raises_runtime_error(self) -> None:
        """Test that `quiet` is rejected as a decorator-level default even though the HTTP client supports it as a
        call-time raw option, since it would collide with the `quiet=quiet` keyword `EndpointFunc._call` always
        passes explicitly
        """
        with pytest.raises(RuntimeError, match="Invalid raw option"):

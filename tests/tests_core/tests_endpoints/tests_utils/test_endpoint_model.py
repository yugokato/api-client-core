"""Unit tests for core endpoints/utils/endpoint_model.py"""
...
import api_client_core.core.endpoints.utils.endpoint_model as endpoint_model_util
from api_client_core.types import Alias, DataclassModelField, File, Kwargs, Unset
...
class TestResolveSignatureName:
    """Tests for `resolve_signature_name()`, the inverse of `alias_illegal_model_field_names()`: mapping a
    (possibly renamed) model field back to the real signature parameter name a name-keyed caller must
    actually dispatch by
    """

    def test_field_name_already_in_signature_is_returned_unchanged(self) -> None:
        """Test that a field name matching a real signature parameter is returned as-is, without even
        inspecting its Alias metadata
        """

        def _func(name: str) -> None: ...

        sig = inspect.signature(_func)
        assert endpoint_model_util.resolve_signature_name("name", str, sig) == "name"

    def test_renamed_field_resolves_via_its_alias_metadata(self) -> None:
        """Test that a field renamed away from its signature name - e.g. a body parameter renamed to
        `user_id_` because it collided with a same-named path parameter, per
        `build_endpoint_model()`'s own path/body collision handling - resolves back to the original via
        its `Alias` metadata
        """

        def _func(user_id: str) -> None: ...

        sig = inspect.signature(_func)
        field_type = Annotated[str, Alias("user_id")]
        assert endpoint_model_util.resolve_signature_name("user_id_", field_type, sig) == "user_id"

    def test_no_alias_metadata_falls_back_to_the_field_name(self) -> None:
        """Test that a field name absent from the signature, with no Alias metadata to resolve it, falls
        back to the field name unchanged rather than raising
        """

        def _func(other: str) -> None: ...

        sig = inspect.signature(_func)
        assert endpoint_model_util.resolve_signature_name("missing", str, sig) == "missing"

    def test_alias_not_matching_any_signature_parameter_falls_back_to_the_field_name(self) -> None:
        """Test that an Alias value that itself doesn't match any real signature parameter falls back to
        the field name unchanged, rather than returning an unresolvable alias
        """

        def _func(other: str) -> None: ...

        sig = inspect.signature(_func)
        field_type = Annotated[str, Alias("still-not-a-param")]
        assert endpoint_model_util.resolve_signature_name("my_param", field_type, sig) == "my_param"

    def test_resolves_through_a_union_of_annotated_types(self) -> None:
        """Test that a union of several `Annotated[]` types (e.g. `Annotated[int, Alias(...)] |
        Annotated[str, Alias(...)]`) resolves via whichever member's `Alias` actually matches the
        signature
        """

        def _func(real_name: str) -> None: ...

        sig = inspect.signature(_func)
        field_type = Annotated[int, Alias("nonexistent")] | Annotated[str, Alias("real_name")]
        assert endpoint_model_util.resolve_signature_name("field", field_type, sig) == "real_name"

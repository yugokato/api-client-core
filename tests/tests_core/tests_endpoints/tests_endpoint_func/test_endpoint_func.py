...
from api_client_core.core.endpoints import AsyncEndpointFunc, Endpoint, EndpointFunc, SyncEndpointFunc, endpoint
...
class TestEndpointFunc:
    ...
    def test_repr(self, api_client: APIClient, api_class: type[BaseAPI]) -> None:
        ...
        repr_str = repr(endpoint_func)
        assert f"endpoint: {endpoint_func.endpoint}" in repr_str
        assert f"mapped to: {endpoint_func._original_func!r}" in repr_str
...
class TestEndpointParamDefinition:
    ...
    def _spy_path_and_params(self, mocker: MockerFixture) -> tuple[Any, Any]:
        mocker.patch.object(Client, "request")
        path_spy = mocker.spy(
            __import__(endpoint_call_util.__name__, fromlist=["complete_endpoint"]),
            "complete_endpoint",
        )
        params_spy = mocker.spy(
            __import__(endpoint_call_util.__name__, fromlist=["validate_params"]),
            "validate_params",
        )
        return path_spy, params_spy

    @staticmethod
    def _body_params(params_spy: Any) -> dict[str, Any]:
        """Extract body/query params from the validate_params spy call args."""
        return params_spy.call_args.args[1]
    ...
    def test_path_positional_body_positional(self, mocker: MockerFixture, api_client: APIClient) -> None:
        ...
        assert path_spy.spy_return == "/v1/customers/1"
        assert self._body_params(params_spy) == {"customer_type": "premium"}
    ...
    def test_multiple_path_params_positional(self, mocker: MockerFixture, api_client: APIClient) -> None:
        ...
        assert path_spy.spy_return == "/v1/customers/10/orders/42"
        assert self._body_params(params_spy) == {"status": "active"}
    ...
    def test_path_param_default_applied_when_omitted(self, mocker: MockerFixture, api_client: APIClient) -> None:
        ...
        assert path_spy.spy_return == "/v1/customers/99"
        assert self._body_params(params_spy) == {}
    ...
    def test_path_param_default_overridden(self, mocker: MockerFixture, api_client: APIClient) -> None:
        ...
        assert path_spy.spy_return == "/v1/customers/5"
        assert self._body_params(params_spy) == {}
    ...
    def test_non_identifier_placeholder_matched_by_cleaned_name_positional(
        ...
        assert path_spy.spy_return == "/v1/customers/42"
        assert self._body_params(params_spy) == {}

...
class APIBase(Generic[APIClientT], metaclass=ABCMeta):
    ...
    def __init_subclass__(cls, **kwargs: Any) -> None:
        ...
        from ..endpoints.endpoint_factory import PendingDecoratorCall
        from ..endpoints.endpoint_handler import EndpointHandler, PendingHandler, PendingOperations

        def wraps_handler(func: Any, _depth: int = 10) -> bool:
            if _depth == 0:
                return False
            if isinstance(inspect.unwrap(func), (EndpointHandler, PendingOperations)):
                return True
            if func.__closure__:
                for cell in func.__closure__:
                    try:
                        cell_content = cell.cell_contents
                        if isinstance(cell_content, (EndpointHandler, PendingOperations)):
                            return True
        ...
        for attr_name, attr in cls.__dict__.items():
            func = attr.func if isinstance(attr, PendingOperations) else attr
            ...
            elif isinstance(attr, PendingDecoratorCall):
                raise RuntimeError(
                    f"{cls.__name__}.{attr_name}: Invalid API function definition. Requires @endpoint.<method>() "
                    f"decorator, or if '{attr_name}' stores a configured decorator (e.g. `{attr_name} = "
                    f"my_decorator(some_arg)`), define it outside the API class body instead."
                )
    ...
    def __init__(self, api_client: APIClientT):
        if self.app_name is not None and self.app_name != api_client.app_name:
            raise ValueError(
...
def get_api_classes(
    api_module_name: str,
    base_api_class: type[APIClassT],
    *,
    class_filter: Callable[[type[APIClassT]], bool] | None = None,
) -> list[type[APIClassT]]:
    """Get all API classes subclassed from the base defined in an API module

    :param api_module_name: Dotted module name of the package containing API class files
    :param base_api_class: Base API class to filter subclasses by
    :param class_filter: Optional predicate to further filter the discovered API classes
    """

...
def create_endpoint_model(
    ...
    model_base: type[EndpointModel] = EndpointModel,
) -> type[EndpointModel]:
    ...
    :param model_base: Base class for the generated model. Defaults to the core `EndpointModel`. Higher layers can
                       inject their own `EndpointModel` subclass to add behavior to every generated model.
    """
    ...
    return build_endpoint_model(
        ...
        field_name_sanitizer=field_name_sanitizer,
...
def resolve_signature_name(field_name: str, field_type: Any, sig: inspect.Signature) -> str:
    """Resolve a model field back to the original signature parameter name it was derived from.

    The inverse of `alias_illegal_model_field_names()`: a model field is renamed away from its signature
    name when it collides with a reserved name or a path parameter of the same name, in which case its
    `Alias` metadata holds the original name. A name-keyed caller (the CLI, the MCP server) driving a
    flag/tool-schema property and the endpoint call by the renamed name would either miss the target
    parameter entirely or have its value silently absorbed into `**kwargs`.

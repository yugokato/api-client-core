...
def split_params(
    func: Callable[..., Any], path: str, args: tuple[Any, ...], kwargs: dict[str, Any]
) -> tuple[tuple[Any, ...], dict[str, Any]]:
    """Split positional and keyword arguments into path parameters and body/query parameters.

    Categorization is based on whether the parameter name matches a `{placeholder}` token in the endpoint path.
    Non-Python-identifier placeholders (e.g. `{customer-id}`) are matched against their cleaned equivalent
    (`customer_id`). Path-parameter defaults are applied when the caller omits them, and a `ValueError` is raised if
    any placeholder remains unresolved.
    Body/query defaults are left to `get_signature_defaults()` so hook contracts remain unchanged.
    ...
    path_params = _resolve_path_params(func, path, named)
    return path_params, named
...
def normalize_call_args(
    ...
    A caller that can only ever produce keyword arguments (e.g. the CLI generator or the MCP server,
    both driven by a name-keyed namespace) needs this to reach a positional-only endpoint parameter,
    since `EndpointFunc._call()`/`split_params()` enforce positional-only-ness exactly like a normal
    Python call would. This is a CLI/MCP-only escape hatch and must never be called from
    `EndpointFunc`/`split_params()` themselves. Doing so would relax positional-only enforcement for
    every caller, not just a name-keyed front end.
...
def complete_endpoint(endpoint: Endpoint[Any], path_params: tuple[str, ...]) -> str:
    """Complete endpoint path with given path variables

    `path_params` is expected to already have exactly one value per path placeholder, which
    `_resolve_path_params()` guarantees before this is ever reached. `zip(..., strict=True)` is
    only a safety net against that invariant being violated.
    ...
    if path_placeholders := list(get_path_placeholders(endpoint.path)):
        fmt = OrderedDict(zip(path_placeholders, path_params, strict=True))
...
def generate_rest_func_params(
    ...
    **raw_options: Any,
    ...
    data = {}
    query_params = {}
    is_json = is_json_request(endpoint, endpoint_params, raw_options, session_headers)
    ...
    for param_name, param_value in endpoint_params.items():
        ...
        if param_name == "raw_options":
            # Already merged into rest_func_params above. Skip so it isn't also treated as a body param
            continue
        if field_obj := dataclass_fields.get(param_name):
            ...
            if annotated_type := param_type_util.get_annotated_type(
                ...
                if isinstance(annotated_type, list | tuple):
                    ...
                    matched = next((t for t in annotated_type if param_type_util.matches_type(param_value, t)), None)
                    if matched is None:
...
def validate_raw_options(raw_options: dict[str, Any]) -> None:
    """Validate raw options against the HTTP client's supported request parameters.

    :param raw_options: Raw request options passed to an endpoint call, or an `@endpoint.<method>()` decorator
    """
    supported_raw_options = set(get_supported_request_parameters()) - {"quiet"}
    invalid_raw_options = set(raw_options).difference(supported_raw_options)
    if invalid_raw_options:
        raise RuntimeError(f"Invalid raw option(s):\n{list_items(invalid_raw_options)}")


def validate_params(endpoint: Endpoint[Any], params: dict[str, Any], raw_options: dict[str, Any] | None) -> None:
    """Validate the endpoint parameters

     A warning message will be logged if any of the following condition matches:
    - The endpoint itself is deprecated
...
def _resolve_path_params(func: Callable[..., Any], path: str, named: dict[str, Any]) -> tuple[Any, ...]:
    """Resolve path parameter values from bound arguments, in path-placeholder order.

    Resolved entries are popped out of `named` in place, leaving only body/query parameters
    behind. Identifying missing placeholders by name (rather than deferring to count-based
    validation later) ensures the resulting error names the actual placeholder(s) the caller
    omitted, not the trailing placeholders in the path.

    :param func: Original API function
    :param path: Endpoint path
    :param named: Bound arguments keyed by parameter name. Path parameter entries are popped out as they are resolved
    """
    sig = get_params_signature(func)
    ...
    placeholders = get_path_placeholders(path)
    if missing := [ph for ph in placeholders if ph not in path_params_dict]:
        missing_str = ", ".join(repr(name) for name in missing)
        raise ValueError(
            f"{func.__name__}() missing {len(missing)} required path parameter{'s' if len(missing) > 1 else ''}: "
            f"{missing_str}"
        )
    return tuple(path_params_dict[ph] for ph in placeholders)
